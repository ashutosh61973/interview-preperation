...
vector<long long> distributeCandies(int N, int K)
{
    ...
    long long last = K * count;

    long long total = last * (last + 1);

    total /= 2;

    N = N - total;

    long long val = last + 1;
    // cout<<N<<endl;
...
*/
    ll k, n;
    cin >> n >> k;
    vector<ll> v = distributeCandies(n, k);
    for (auto x : v)
    {
        cout << x << " ";
    }
